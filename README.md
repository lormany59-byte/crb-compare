...
ດັບເບິລຄິກໄຟລ໌ **`run.bat`** — ມັນຈະຕິດຕັ້ງ dependencies, ເປີດ server,
ແລະ ເປີດ browser ໃຫ້ອັດຕະໂນມັດ.

> ⚠️ **ຢ່າດັບເບິລຄິກ `index.html` ໂດຍກົງ** — ໜ້າຈະເພ (ຂຶ້ນໂຄ້ດ `{% %}` ດິບ
> ແລະ ປຸ່ມໃຊ້ບໍ່ໄດ້) ເພາະ Python server ບໍ່ໄດ້ run. ຕ້ອງເຂົ້າຜ່ານ
> `http://127.0.0.1:5000` ສະເໝີ.

### 📦 ແຈກໃຫ້ພະນັກງານ (ບໍ່ຕ້ອງຕິດຕັ້ງ Python)

ສ້າງໄຟລ໌ **`.exe` ດຽວ** ທີ່ພະນັກງານພຽງດັບເບິລຄິກກໍໃຊ້ໄດ້ — ບໍ່ຕ້ອງມີ Python ໃນເຄື່ອງ:

1. ດັບເບິລຄິກ **`build_exe.bat`** (ຕ້ອງມີ Python ສະເພາະ **ເຄື່ອງທີ່ build** ເທົ່ານັ້ນ)
2. ໄດ້ໄຟລ໌ທີ່ `dist\CRB-Compare.exe`
3. ກ໊ອບປີ້ **`CRB-Compare.exe`** ໄຟລ໌ດຽວໄປໃຫ້ພະນັກງານ → ດັບເບິລຄິກ → browser ເປີດ
   `http://127.0.0.1:5000` ໃຫ້ເອງ

> ໝາຍເຫດ: ໄຟລ໌ .exe ~34MB, ຄັ້ງທຳອິດເປີດອາດຊ້າ 5-10 ວິ (ກຳລັງແຕກໄຟລ໌).
> ຖ້າ Windows SmartScreen ເຕືອນ → "More info" → "Run anyway".
